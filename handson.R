...
ishigaki_R4 <- read_excel("data/ishigaki.xlsx") |>
  ...
  mutate(causes = factor(causes,
                         levels = c("Other", "Liver cirrhosis", "Tuberculosis", "Diabetes mellitus", "Hypertensive disease", "Unforeseen accident", "Pneumonia", "Suicide", "Cerebrovascular disease", "Senility", "Heart disease", "Cancer"),
                         labels = c("その他", "肝硬変", "結核", "糖尿病", "高血圧性疾患", "不慮の事故", "肺炎", "自殺", "脳血管疾患", "老衰", "心疾患", "悪性新生物")))

# Draw bar chart
ggplot(ishigaki_R4, aes(x = num, y = causes)) +
  geom_bar(stat = "identity", fill = "black") +
  geom_text(aes(x = num + 10, y = causes), label = ishigaki_R4$num, size = 8) +
  labs(x = "人数", y = "死因") +
...
ishigaki <- read_excel("data/ishigaki.xlsx") |>
  mutate(causes = factor(causes,
                         levels = c("Cancer", "Heart disease", "Senility", "Cerebrovascular disease", "Suicide", "Pneumonia", "Unforeseen accident", "Hypertensive disease", "Diabetes mellitus", "Tuberculosis", "Liver cirrhosis", "Other"),
